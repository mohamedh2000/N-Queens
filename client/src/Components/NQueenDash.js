...
const NQueenDash = () => {
    ...
    return (
            <div className="flex flex-col space-y-10 w-full h-full justify-center items-center p-5 rounded-2xl">
                {! (inputVal > 0 && inputVal <= 9) ? <i className="text-red">Please input a Number between 1 and 9</i>: <></>}
                <div className="w-1/3 min-w-[180px] max-h-[210px] h-full flex flex-col justify-center items-center space-y-3 shadow-2xl p-5 rounded-2xl">
                    ...
                    <input className="shadow-2xl rounded-full w-full p-3 outline-none text-center" max={9} min={1} type="number"
                ...
                <ul className={`flex flex-row h-full space-x-5 w-full overflow-x-auto ${solutions.length <= 1 ? ' text-center justify-center items-center' : ''}`}>
